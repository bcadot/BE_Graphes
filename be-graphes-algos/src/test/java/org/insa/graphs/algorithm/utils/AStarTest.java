...
public class AStarTest {
	...
	public void testEmptyGraph() {
		for (int i = 0; i < 5; i++) {
			for (int j = 0; j < 5; j++) {
				ShortestPathSolution solution = new AStarAlgorithm(new ShortestPathData(emptygraph, nodes[i], nodes[j], arc0)).run();
	...
	public void testIslandPath() {
		for (int i = 0; i < 5; i++) {
			ShortestPathSolution solution = new AStarAlgorithm(new ShortestPathData(graph, nodes[i], nodes[5], arc0)).run();
			assertEquals(Status.INFEASIBLE, solution.getStatus());
		}
	}
