...
public class Path {
	...
	public boolean isValid() {
		boolean valid = true;
		boolean firstnode = true;
		if (!this.isEmpty() && !( this.getArcs().isEmpty() && (this.size() == 1) )) {
			Node destprecedent = this.getArcs().get(0).getDestination();
			for (Arc arc : this.getArcs()) {
				if ((arc.equals(this.getArcs().get(0))) && firstnode) {
					valid &= (this.getOrigin() == arc.getOrigin());
					firstnode = false;
				} else if (destprecedent == arc.getOrigin()) {
					valid &= true;
					destprecedent = arc.getDestination();
				} else {
					valid = false;
					break;
				}
